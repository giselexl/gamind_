...
import { db, auth } from '../firebaseConfig'; // Importado o "auth" para vincular o criador da lista
...
export default function CreateListScreen() {
  ...
  // Obtém as informações do usuário logado no momento
  const usuarioAtual = auth.currentUser;

  const handleSelecionarJogo = (jogo: any) => {
    if (!nomeLista.trim()) {
      if (Platform.OS === 'web') window.alert("Preencha o nome da lista primeiro.");
      else Alert.alert("Atenção", "Preencha o nome da lista primeiro.");
      return;
    }
    
    if (!descricaoLista.trim()) {
      if (Platform.OS === 'web') window.alert("Preencha a descrição da lista primeiro.");
      else Alert.alert("Atenção", "Preencha a descrição da lista primeiro.");
      return;
    }
  ...
  const handleCriarLista = async () => {
    if (!usuarioAtual) {
      if (Platform.OS === 'web') window.alert("Usuário não autenticado.");
      else Alert.alert("Erro", "Você precisa estar logado para criar uma lista.");
      return;
    }
    ...
    try {
      ...
      if (Platform.OS === 'web') window.alert("Não foi possível criar a lista.");
      else Alert.alert("Erro", "Não foi possível criar a lista.");
    } finally {
      setSalvando(false);
    }
  };
